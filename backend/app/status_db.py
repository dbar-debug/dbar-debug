...
Дані величезні (~710 МБ zip, десятки млн рядків — по кілька стадій на
справу), тож будуємо через диск, а не памʼять:
  1) raw   — усі рядки стадій потоково;
  2) cases — остання стадія по кожній справі (поточний стан) + найповніші
             сторони (для пошуку за ПІБ);
  3) cases_fts — FTS5 по сторонах.
Потім raw видаляється, база стискається (VACUUM) й атомарно підмінює стару.

Колонки джерела: court_name, case_number, case_proc, registration_date,
judge, judges, participants, stage_date, stage_name, cause_result,
cause_dep, type, description.
"""
...
def build(rows: Iterable[tuple]) -> int:
    """
    Будує базу у тимчасовий файл і атомарно підмінює DB_PATH.
    `rows` — ітерабель кортежів у порядку IN_COLS.
    Повертає кількість унікальних справ.
    """
    ...
    try:
        conn.executescript(
            ...
            PRAGMA cache_size = -200000;   -- ~200 МБ кеш
            CREATE TABLE raw (
        ...
        # 1) Потокова вставка всіх стадій
        cur = conn.cursor()
        batch: List[tuple] = []
        total = 0
        for r in rows:
            # r у порядку IN_COLS (11 полів); додаємо stage_date_iso
            iso = _date_iso(r[7])
            batch.append((r[0], r[1], r[2], r[3], r[4], r[5], r[6], r[7], iso, r[8], r[9], r[10]))
            if len(batch) >= 50000:
                cur.executemany(
                    "INSERT INTO raw VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", batch
                )
                total += len(batch)
                batch.clear()
                if total % 1_000_000 == 0:
                    print(f"[status] вставлено рядків: {total:,}")
        if batch:
            cur.executemany("INSERT INTO raw VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", batch)
            total += len(batch)
        conn.commit()
        print(f"[status] Усього рядків стадій: {total:,}. Згортаю до справ...")

        # 2) Остання стадія по справі + найповніші сторони
        conn.executescript(
            """
            CREATE TABLE cases (
            ...
            INSERT INTO cases_fts(rowid, participants)
                SELECT id, participants FROM cases;
            """
        )
